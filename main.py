...
class RSN:
	def Nfund(): return "Tài khoản không tồn tại"
	def sentd():return '"message":"success"'
	def BAND(): return 'Đã đạt giới hạn tối đa số lần thử. Vui lòng thử lại sau.'
	def ErrEml(): return 'Nhập địa chỉ email hợp lệ'
class TIKTOK_EMAILS:
	def __init__(self):
		self.PROXS,self.ERROR,self.DONE=0,0,0
		self.Loks=Lock();self.RUN=True
		self.emails='joker@221298.vv1ck'
		try:self.All=open(input('\n[+] Nhập tên file email: '),'r')
		except FileNotFoundError:print('\n[!!] Tên file không hợp lệ, vui lòng thử lại ..\n');return TIKTOK_EMAILS()
		try:self.proxy =  open(input('\n[$] ->> Nhập tên file (Proxy) : '),'r').read().splitlines()
		except FileNotFoundError:print('\n[!!] Tên file không hợp lệ, vui lòng thử lại ..\n');return TIKTOK_EMAILS()
		print('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n')
	...
	def PRNT(self):
		while self.RUN:
			try:print(f'\r - Đã kiểm tra-TIK:[{self.DONE}], Lỗi:[{self.ERROR}], proxies:[{self.PROXS}] |{self.emails}\r',end="")
